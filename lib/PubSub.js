...
class PubSub extends Client {
  ...
  async subscribe(topic, subscriber) {
    ...
    const callback = async (msg) => {
      ...
      if (!msg) {
        // If the consumer was cancelled remotely we will not get anymore messages,
      ...
      let message;
      try {
        message = Client.decode(msg.content);
      } catch (err) {
        this.emit('error', err);
        // Acknowledge invalid messages to prevent them from being requeued.
        await this.ack(msg);
      }
      try {
